/**
 * Ручная синхронизация с исправлением room_id и дат
 */
...
/**
 * Преобразовать Channex бронирование в формат PMS с правильными датами
 */
async function convertToPMSFormat(channexBooking) {
  ...
  console.log('📅 Даты из attrs:', { 
    arrival: attrs.arrival_date, 
    departure: attrs.departure_date,
  ...
  const pmsBooking = {
    ...
    total_amount: parseFloat(room?.amount || attrs.amount || '300'),
...
async function testManualSync() {
  console.log('🧪 Ручная синхронизация с исправлениями');
  console.log('📋 Booking ID:', BOOKING_ID);
  
  try {
    // 1. Получаем бронирование из Channex
    const response = await channexApiRequest(`/bookings/${BOOKING_ID}`);
    const channexBooking = response.data;
    console.log('📥 Получено из Channex:', channexBooking?.id);
    
    // 2. Конвертируем в формат PMS с исправлениями
    const pmsBooking = await convertToPMSFormat(channexBooking);
    ...
    console.log('- Комната:', `${result[0]?.room_number} (room_id: ${result[0]?.room_id})`);
