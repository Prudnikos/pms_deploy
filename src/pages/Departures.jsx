...
import { Calendar, User, Phone, Mail, MapPin, Search, Filter, LogOut, CheckCircle } from 'lucide-react';
...
export default function Departures() {
  ...
  const fetchDepartures = async () => {
    ...
    try {
      ...
      // Фильтруем только выезды на выбранную дату
      const departures = (result.data || []).filter(booking => {
        if (booking.status === 'cancelled') return false;
        const checkOut = format(parseISO(booking.check_out), 'yyyy-MM-dd');
        return checkOut === selectedDate;
      });

      console.log('📅 Departures for', selectedDate, ':', departures);
  ...
  const handleCheckOut = async (booking) => {
    // Здесь будет логика отметки выезда
    console.log('✅ Check-out:', booking.guests?.full_name);
    // TODO: Обновить статус брони на 'checked_out'
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Выезд: {format(parseISO(booking.check_out), 'HH:mm')}
                </p>
              </div>
            </div>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Выезды</h1>
          <p className="text-slate-600 mt-1">
            Управление выездами гостей
          </p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-slate-600">Загрузка выездов...</p>
        </div>
        ...
        <Card className="p-8">
          <div className="text-center text-slate-500">
            <LogOut className="h-12 w-12 mx-auto mb-4 text-slate-300" />
            <h3 className="text-lg font-semibold mb-2">Нет выездов</h3>
            <p>На {format(new Date(selectedDate), 'dd MMMM yyyy', { locale: dateLocale })} выездов нет</p>
          </div>
        </Card>
      )}
